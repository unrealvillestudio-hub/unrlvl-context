...
> **Nota de sincronización — 2026-09-23 (`ecosystem.json` v2026-09-21-v1 → **v2026-09-23-v1**).**
> ****Este archivo se SINCRONIZA, no se regenera, y el cuerpo previo queda íntegro.****
>
> **Nodos que cambiaron en el JSON, y NADA MÁS** —comprobado nodo a nodo comparando el árbol
> aplanado del `HEAD` anterior con el nuevo: **24 rutas nuevas, 32 modificadas y CERO eliminadas**;
> las 23 «modificadas» de `previous_chain` son el desplazamiento de añadir una versión al tope, no
> una pérdida—:
>
> 1. **`_meta`** — `version` → **`2026-09-23-v1`**, `previous` → `2026-09-21-v1`, `previous_chain`
>    con `2026-09-21-v1` al tope. El `last_session` anterior (**2026-09-19**, que las sesiones del
>    21 y del 22 no tocaron) se **archiva íntegro** bajo `_last_session_archivo_2026-09-23-v1`
>    **sin reescribir su texto**, y `previous_sessions` **no se toca** — mismo criterio que
>    `_last_session_archivo_2026-09-10-v1` y `_actualiza_2026-09-17-v1`. Clave nueva
>    `_actualiza_2026-09-23-v1` declarando esta misma lista.
> 2. **`supabase.main.schemas.alerting.functions`** — de **11 a 13**: altas de
>    **`alerting.watchdog_beat`** y **`alerting.purge_cron_run_details`**, las dos
>    `SECURITY DEFINER` **con su `REVOKE … FROM PUBLIC`** (`CC_PROTOCOL` §11). La entrada de
>    `watchdog_snapshot` **amplía** su descripción con la corrección del `Seq Scan`; no se
>    reescribe lo anterior.
> 3. **`supabase.main.schemas.alerting.tables_list`** — dos entradas **amplían**: la de
>    `watchdog_heartbeat` dice ahora **quién la escribe** y declara su limitación abierta —es **una
>    sola fila que se sobrescribe**, así que no hay historia—; la de `cron_run_rollup` anota que su
>    tabla de origen **se purga a diario**.
> 4. **`supabase.main.schemas.alerting.migraciones`** — de **5 a 6 pines**, con
>    `20260923200000_el_vigilante_late_aunque_la_foto_falle.sql`. **Anotado abreviado a propósito:**
>    el blob sha completo vive en `supabase/MIGRACIONES_CONGELADAS.md` de `unrlvl-iid-functions`,
>    que es la fuente, y una segunda copia entera podría divergir.
> 5. **`alerting_channel.crons`** — alta de **`alerting-purga-historial-cron-daily`** (`25 3 * * *`,
>    jobid 117, activo [`medido`]).
> 6. **`alerting_channel.vigilante_externo`** — **amplía**: el beat va **antes** de la foto, el
>    estado del latido viaja en las dos salidas y el aviso por la vía directa distingue «falla la
>    foto» de «no responde nada».
> 7. **`alerting_channel._actualiza_2026-09-23`** — **clave nueva** con el diagnóstico completo del
>    503, los dos errores de diagnóstico de CC y la corrección de Sam que no procedía.
> 8. **`iid_subsystem._techo_de_caracteres_y_cuota_de_imagen_2026-09-23`** — **clave nueva**, con
>    `HR-GEN-14`, `HR-GEN-15`, el contrato de `builder_input` a 21 claves, la coherencia de techos,
>    `analysis_lenses`, la cuota de imagen con su ventana en el dato y el incidente de `pass_type`.
>
> **Verificado:** el diff de `ecosystem.json` **no borra ni un solo valor** —cero rutas eliminadas—,
> y por tanto **ninguna sección del cuerpo de abajo se reescribe**. Las entradas 2, 3 y 6 **amplían**
> texto existente; el resto son altas.
>
> **No existe generador en este repositorio**, así que «regenerar» a mano no sería regenerar: sería
> reescribir con interpretación —justo lo que la instrucción de cero interpretación busca impedir— y
> borraría historia, que es la regla suprema (`CC_PROTOCOL` §0). Estos dos archivos llevan además
> **cuerpo acumulado que no es derivable del JSON** —flujos, tablas de estado, notas fechadas—: una
> regeneración literal los vaciaría.
>
> **Nota de sincronización anterior íntegra inmediatamente debajo.**

> **Nota de sincronización — SEC-03, 2026-09-21 (`ecosystem.json` v2026-09-20-v2 → **v2026-09-21-v1**).**
> ****Este archivo se SINCRONIZA, no se regenera, y el cuerpo previo queda íntegro.****
>
> **Nodos que cambiaron en el JSON, y NADA MÁS:**
>
> 1. **`security.SEC-03`** — **entrada nueva**, con la forma del nodo hermano `SEC-02`. Severidad
>    **ROJO**, estado **ABIERTO**: la Edge Function `media-store` lleva un secreto **cableado como
>    valor por defecto** de `Deno.env.get(...)`, con `verify_jwt: false` y escritura con
>    `service_role` [`medido` el 2026-09-21]. **El valor NO se transcribe**, por `CC_PROTOCOL` §15.
>    Entra al registro porque el `security` del ecosistema es donde vive esta clase de deuda, y sin
>    la entrada un `ecosystem scan` no la vería.
> 2. **`_meta`** — `version` → **`2026-09-21-v1`**, `previous` → `2026-09-20-v2`, `previous_chain`
>    con `2026-09-20-v2` al tope. **`last_session` NO se toca:** la escribió el Actualiza del
>    2026-09-20 y sigue siendo exacta para esa sesión. Se declara para que la ausencia **no se lea
>    como olvido**. `previous_sessions` no cambia.
>
> **Verificado:** el diff de `ecosystem.json` **borra exactamente dos líneas** —`version` y
> `previous`—, las dos de valor. **Ningún otro nodo cambió**, y por tanto **ninguna sección del
> cuerpo de abajo se reescribe**.
>
> **El resto del Actualiza del 2026-09-21 NO tocó `ecosystem.json`** —el playbook del Sales-Kit,
> `CC_PROTOCOL` §15 y `CAPABILITIES` 1.22 son documentación—, y por eso esta nota sólo habla de SEC-03.
>
> **Por qué no se regenera:** **no existe generador en el repo**, así que «regenerar» a mano es
> **reescribir con interpretación** y **borra historia** (`CC_PROTOCOL.md` §0). Regla en `CLAUDE.md`.
>
...
> **Nota de sincronización — KEEPALIVE VERIFICADO Y CRON INTERNO RETIRADO, 2026-09-20 (`ecosystem.json` v2026-09-19-v1 → **v2026-09-20-v1**).**
> ****Este archivo se SINCRONIZA, no se regenera, y el cuerpo previo queda íntegro.****
>
> **Nodos que cambiaron en el JSON, y NADA MÁS:**
>
> 1. **`keepalive_externo`** — dos campos corregidos porque **habían dejado de ser ciertos**:
>    `_status` pasa a **VERIFICADO POR EFECTO** (los **tres** disparos programados aterrizaron:
>    2026-09-19 18:11:01, 2026-09-20 02:11 y 10:11 UTC), y `_abierto` registra que el **cron interno
>    `jobid 1` fue RETIRADO** el 2026-09-20 junto con su función, después del primer latido y con
>    guarda. **La tabla no se tocó:** conserva sus 4 filas históricas.
> 2. **`_meta`** — `version` → **`2026-09-20-v1`**, `previous` → `2026-09-19-v1`, `previous_chain`
>    con `2026-09-19-v1` al tope. **`last_session` NO se toca, y es deliberado:** describe la sesión
>    del 2026-09-19 y sigue siendo exacta; este retiro es su continuación y vive en el nodo, en
>    `AGENDA.md` y en `brands/ForumPHs/session_log.md`. Se declara para que la ausencia **no se lea
>    como olvido**. `previous_sessions` sigue en 20.
>
> **Verificado:** el diff de `ecosystem.json` **borra exactamente cuatro líneas** —`_status`,
> `_abierto`, `version` y `previous`— y **las cuatro son de valor**. Ningún otro nodo cambió, y por
> tanto **ninguna sección del cuerpo de abajo se reescribe**.
>
> **Por qué no se regenera:** **no existe generador en el repo**, así que «regenerar» a mano es
> **reescribir con interpretación** y **borra historia** (`CC_PROTOCOL.md` §0). Regla en `CLAUDE.md`.

> **Nota de sincronización — KEEPALIVE EXTERNO, 2026-09-19 (`ecosystem.json` v2026-09-17-v1 → **v2026-09-19-v1**).**
> **Este archivo se SINCRONIZA, no se regenera, y el cuerpo previo queda íntegro.**
>
> **Nodos que cambiaron en el JSON, y NADA MÁS:**
>
> 1. **`_meta`** — `version` → `2026-09-20-v2`, `previous` → `2026-09-20-v1`, y `2026-09-20-v1` entra
>    **al tope** de `previous_chain`. Es el **segundo** corte del 2026-09-20: el `v1` de esa misma
>    fecha es el del keepalive, de otra sesión, y queda íntegro. Ninguna entrada de la cadena se pierde.
> 2. **`alerting_channel.edge_functions.ops-daily-report`** — el registro pasa a declarar **DOS
>    MODOS**: cuerpo vacío o `mode:daily` es el informe diario de las 07:00 UTC de siempre;
>    **`mode:digest`** es el **resumen de cada 4 horas**, tres secciones y **sólo conteos** —ni lista de
>    piezas ni un solo `piece_id`, porque la lista es de la bandeja, que es donde se actúa sobre ella—.
>    Los dos cuentan con **las mismas funciones**: dos servicios que cuentan lo mismo por su cuenta
>    terminan dando dos cifras del mismo hecho. Un modo desconocido devuelve **400** en vez de mandar
>    el informe equivocado en silencio.
> 3. **`alerting_channel.edge_functions.iid-approval-digest`** — **entrada nueva**. El despertador de
>    las 7am ET, con guarda DST-proof, **adoptada al repositorio `unrlvl-iid-functions` el 2026-09-20**:
>    llevaba **ACTIVA en producción desde julio en su versión 28** y **su código no existía en ninguna
>    rama** [`medido`], que es lo que desvió su diagnóstico a otro repositorio durante días.
> 4. **`alerting_channel.crons.alerting-digest-4h`** — **entrada nueva**, `0 1,5,9,13,17,21 * * *`.
>    Ninguna franja coincide con las 07:00 del informe diario: dos correos a la vez se leen como uno.
> 5. **`criterio_de_bandeja`** — **NODO NUEVO**, y es el eje del día: **qué pieza espera un veredicto
>    humano**. Su fuente única (`content.pieces_awaiting_criterion`), sus dos reglas —**el estado vivo
>    de una fila es su `status`, las columnas `*_at` son historia**, y **el criterio no es una
>    cadena**—, los tres estados que esperan, los **dos `challenged` distintos** (el del juez se
>    arbitra, el de Sam se arregla), el grano del aviso en `piece_email_mode`, dónde se guarda cada
>    tramo, por dónde sale y **qué lo sostiene**: los tests que fallan si las tres declaraciones
>    divergen.
>
> **Cuerpo íntegro, cero regeneración.** El motivo no ha cambiado desde el 2026-08-13: **no existe
> generador en el repo**, así que «regenerar» a mano no es regenerar — es reescribir con interpretación
> y borrar historia (`CC_PROTOCOL.md` §0). Y este archivo lleva cuerpo acumulado que **no es derivable**
> del JSON —flujos, tablas de estado y notas fechadas— que una regeneración literal vaciaría. **Es la
> regla escrita en `CLAUDE.md`** desde el 2026-08-23, no una excepción. **Commit separado.**
> 1. **`keepalive_externo`** — **nodo nuevo de primer nivel**, hermano de `alerting_channel`, escrito
>    con la forma de ese nodo hermano. El latido contra la pausa del plan gratuito: vive en
>    `unrlvl-ops` (`api/keepalive.ts`, cron `11 2,10,18 * * *`), hace un **`POST`** a la REST API de
>    cada objetivo —que es a la vez la actividad que cuenta y el rastro—, y **no nombra ni un
>    proyecto, ni una URL, ni una clave, ni una marca**: los objetivos entran por `KEEPALIVE_TARGETS`.
>    Es **eje**, no instancia, y por eso vive en `unrlvl-ops` y no en `forumphs-ops`.
> 2. **`_meta`** — `version` → **`2026-09-19-v1`**, `previous` → `2026-09-17-v1`, `previous_chain`
>    con `2026-09-17-v1` al tope, y el `last_session` anterior **archivado íntegro** en la clave
>    nueva `_last_session_archivo_2026-09-19-v1` **antes** de escribir el nuevo del **2026-09-19**.
>    `previous_sessions` **no se toca** (sigue en 20). Verificado: el diff de `ecosystem.json`
>    **borra exactamente dos líneas** —`version` y `previous`— y añade 23.
>
> **Ningún otro nodo cambió**, y por tanto **ninguna sección del cuerpo de abajo se reescribe**.
> La sesión de la mañana del 2026-09-19 —el análisis financiero de ForumPHs, `AGENDA.md`
> v2026-09-19-v1— **no tocó `ecosystem.json`** y por eso no dejó nota acá; su detalle vivo está en
> `brands/ForumPHs/session_log.md` y en `AGENDA.md`.
>
> **Por qué no se regenera, y no es una excepción de esta vez:** **no existe generador en el repo**,
> así que «regenerar» a mano es **reescribir con interpretación** —lo que la instrucción de cero
> interpretación busca impedir— y **borra historia**, que es la regla suprema (`CC_PROTOCOL.md` §0).
> Este archivo lleva además cuerpo acumulado **no derivable** del JSON. Regla escrita en `CLAUDE.md`.

> **Nota de sincronización — ALERTAS-01, 2026-09-17 (`ecosystem.json` v2026-09-12-v1 → **v2026-09-17-v1**).**
> **Este archivo se SINCRONIZA, no se regenera, y el cuerpo previo queda íntegro.**
>
> **Nodos que cambiaron en el JSON, y NADA MÁS:**
>
> 1. **`_meta`** — `version` → **`2026-09-17-v1`**, `previous` → `2026-09-12-v1`, `previous_chain`
>    con `2026-09-12-v1` al tope, y el `last_session` anterior **archivado íntegro** en la clave
>    nueva `_last_session_archivo_2026-09-17-v1` **antes** de escribir el nuevo del **2026-09-17**.
>    `previous_sessions` **no se toca** (sigue en 20). Verificado: el diff de `ecosystem.json`
>    **elimina exactamente dos líneas** —`version` y `previous`— y añade 75.
>    **La sesión del 2026-09-13 no bumpeó `_meta`** —su `AGENDA.md` v2026-09-13-v1 lo declara:
>    «`ecosystem.json` no cambió, así que sus derivados no se tocan»—, y por eso `previous` salta
>    desde `2026-09-12-v1`. **CC no le fabrica entrada**: su detalle vivo está en `AGENDA.md`
>    v2026-09-13-v1 y -v2 y en `IID/session_log.md` (2026-09-13).
> 2. **`supabase.main.schemas.alerting`** — **esquema nuevo**, escrito con la forma del nodo hermano
>    **`mail`** y sin tocar ninguna clave previa de `schemas`. Trae sus **13 tablas**
>    (`alert_rules` —con la columna nueva **`silent boolean NOT NULL DEFAULT false`**—,
>    `failure_remediation`, `alert_channels`, `alert_operators`, `alert_routes`, `alert_events`,
>    `alert_action_log`, `cron_watch_exceptions`, `daily_reports`, `security_pending`,
>    `watchdog_heartbeat`, `cron_run_rollup` y `cron_rollup_watermark`), sus **11 funciones**
>    (`evaluate_rules`, `open_event`, `row_matches`, `next_public_ref`, `dispatch_async`,
>    `config_sin_secretos`, `on_source_insert`, `check_watchdog_heartbeat`, `tick_async`,
>    `watchdog_snapshot` y `refresh_cron_run_rollup`), **RLS en las 13 con políticas `service_only`**,
>    el motivo escrito de `verify_jwt: false` —quien llama es `pg_net`, que **no manda JWT de
>    usuario**— y **los cinco pines de migración** tal como figuran en
>    `supabase/MIGRACIONES_CONGELADAS.md`.
> 3. **`alerting_channel`** — **nodo nuevo de primer nivel**, con la forma de **`carril_regulator`**.
>    El canal de alertas: su eje declarado (**un canal de salida, no una consola** — avisa; la única
>    entrada son dos botones sobre el episodio ya abierto), las **tres Edge Functions**
>    (`ops-alert-dispatch` **v7** `ezbr_sha256` `3eb20681…e79142`, `ops-alert-callback`,
>    `ops-daily-report`), los **tres crons** (`alerting-tick-1min`, `alerting-daily-report` a las
>    **07:00 UTC**, `alerting-watchdog-heartbeat-5min`), los **tres disparadores** `AFTER INSERT`
>    sobre `intel.watcher_log`, `intel.brand_publish_drain_log` y `alerting.alert_action_log`, el
>    **vigilante externo** de `unrlvl-ops` —`api/watchdog.ts`, `api/_ventana.mjs`, cron de Vercel
>    `*/5 * * * *`, `/api` excluido del rewrite catch-all, **cero cambios en `src/`**—, las
>    **claves de configuración nombradas por su clave y nunca por su valor**, las decisiones de Sam
>    del 2026-09-17, el estado de la verificación y los cuatro frentes abiertos.
>
> **Ningún valor de secreto entra en el JSON:** `alerting_watchdog_secret`, `CRON_SECRET`,
> `WATCHDOG_SECRET`, `TELEGRAM_BOT_TOKEN`, `ALERT_WEBHOOK_SECRET` y `TELEGRAM_CHAT_ID` se nombran
> **por su clave**. Las carga **Sam**: no existe tool de escritura de secretos ni de variables de
> entorno en ningún MCP de esta sesión [`medido` contra `CAPABILITIES.md` el 2026-09-17].
>
> **Lo demás de este documento no se tocó**, incluido el cuerpo acumulado que **no es derivable del
> JSON** —flujos, tablas de estado y notas fechadas—, que una regeneración literal vaciaría. **No
> existe generador en el repo**: «regenerar» a mano sería reescribir con interpretación y borrar
> historia (`CC_PROTOCOL.md` §0). Es **la regla escrita en `CLAUDE.md`** desde el 2026-08-23, no una
> excepción.

> **Nota de sincronización — CIERRE 2026-09-12 (`ecosystem.json` v2026-09-10-v1 → **v2026-09-12-v1**).**
> **Este archivo se SINCRONIZA, no se regenera, y el cuerpo previo queda íntegro.**
>
> **Nodos que cambiaron en el JSON, y NADA MÁS:**
>
> 1. **`_meta`** — `version` → **`2026-09-12-v1`**, `previous` → `2026-09-10-v1`, `previous_chain`
>    con `2026-09-10-v1` al tope, y el `last_session` anterior **archivado íntegro** en la clave
>    nueva `_last_session_archivo_2026-09-10-v1` **antes** de escribir el nuevo del **2026-09-12**.
>    Verificado por barrido estructural: el diff **pierde CERO claves** respecto de `main` y **añade
>    21**.
> 2. **`carril_regulator`** — **nodo nuevo de primer nivel**. El regulador de entrada al carril:
>    su eje declarado (**una válvula, no un grifo** — no produce, decide si deja pasar lo que ya
>    existe), la EF `carril-regulator` v1 desplegada el 2026-09-12, sus tres tablas
>    (`intel.carril_regulation_log`, `intel.brand_production_margin` y la vista
>    `intel.v_carril_cobertura`), sus dos crons (`carril-regulator-daily` **activo**,
>    `carril-cobertura-alarma-daily` **apagado a propósito**), el `dry_run` del 2026-09-12
>    —**16 canales, 14 `SUPPLY_ABSENT`, 2 `HOLD`, déficit total 75,2**—, el motivo escrito de por
>    qué la alarma sigue apagada con su **condición de encendido declarada por adelantado**, cuál de
>    las fuentes de cadencia manda, y el frente abierto de la reconciliación `iid_agents` ↔ `cron.job`.
> 3. **`publish_pipeline`** — clave nueva **`_update_2026-09-12`**, **aditiva**: ninguna clave previa
>    del nodo se toca. Registra que **N10 está aplicado en la base y pendiente en la función**
>    —DDL sí, despliegue no—, por qué el SQL **no filtra por proveedor**, y las dos columnas nuevas
>    de la vista de cobertura, que **cuentan el sello** y no deciden drenabilidad.
>
> **Lo demás de este documento no se tocó**, incluido el cuerpo acumulado que **no es derivable del
> JSON** —flujos, tablas de estado y notas fechadas—, que una regeneración literal vaciaría. **No
> existe generador en el repo**: «regenerar» a mano sería reescribir con interpretación y borrar
...
> **Este archivo NO cambia en su cuerpo, y se declara.** El cambio de la pasada es una **decisión de
> gobernanza**, no de estado del ecosistema: se instala `protocols/DELIVERY_AND_VERIFICATION_RULE.md`
