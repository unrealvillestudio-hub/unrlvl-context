...
> ****El cuerpo de este archivo NO se regenera, y se declara.****
>
...
> **El cuerpo de este archivo NO se regenera, y se declara.**
>
> **Lo que cambia en el árbol de los repos, y sólo esto** —ninguno de estos archivos vive en
> `unrlvl-context`, así que este mapa los **registra**, no los contiene:
>
> 1. **`unrlvl-iid-functions`** — **archivos nuevos:** `supabase/functions/ops-alert-dispatch/`,
>    `supabase/functions/ops-alert-callback/` y `supabase/functions/ops-daily-report/`; las **cinco
>    migraciones** `20260916123000`, `20260916140000`, `20260917200000`, `20260917210000` y
>    `20260917220000`, **las cinco pineadas en `supabase/MIGRACIONES_CONGELADAS.md`** y verificadas
>    por `tests/migraciones_congeladas_test.mjs`; y los tests
>    `tests/canal_alertas_test.mjs`, `tests/vigilante_silencio_sin_ventana_test.mjs` y
>    `tests/vigilante_resumen_de_corridas_test.mjs`. Suite del repo: **90 ok / 0 fail** [`medido`].
> 2. **`unrlvl-ops`** — **archivos nuevos:** `api/watchdog.ts` y `api/_ventana.mjs`, más
>    `tests/watchdog_ventana_test.mjs` (**17 ok / 0 fail** [`medido`]). **Modificado:** `vercel.json`
>    —cron `*/5 * * * *` y **`/api` excluido del rewrite catch-all**, sin lo cual el catch-all se come
>    la ruta—. **Cero cambios en `src/`.**
>
> **Nodos del JSON que cambiaron**, y que este mapa refleja sin reescribir su cuerpo: `_meta`
> (versión, `previous`, cadena, y el `last_session` anterior **archivado íntegro** en
> `_last_session_archivo_2026-09-17-v1`; `previous_sessions` intacto), el esquema nuevo
> **`supabase.main.schemas.alerting`** —13 tablas, 11 funciones, la columna `silent`, RLS
> `service_only` y los cinco pines de migración—, y el nodo nuevo de primer nivel
> **`alerting_channel`** —tres Edge Functions, tres crons, tres disparadores, el vigilante externo y
> las claves de configuración **por su nombre**—.
>
> **Ningún valor de secreto entra en este archivo ni en el JSON:** las claves se nombran, los valores
> los carga **Sam**.
>
> **Cuerpo íntegro, cero regeneración:** este archivo lleva notas fechadas y estructura acumulada que
> **no es derivable del JSON**, y **no existe generador en el repo** — «regenerar» a mano sería
> reescribir con interpretación y borrar historia (`CC_PROTOCOL.md` §0). Regla escrita en
> `CLAUDE.md` desde el 2026-08-23.

> **Nota de sincronización — CIERRE 2026-09-12 (`ecosystem.json` v2026-09-10-v1 → **v2026-09-12-v1**).**
> **El cuerpo de este archivo NO se regenera, y se declara.**
>
> **Lo que cambia en el árbol del repo, y sólo esto:**
>
> 1. **`skills/publicacion-operativa/SKILL.md` — ARCHIVO NUEVO.** v1.0, 352 líneas, capa **MÉTODO**
>    y destino **CARGABLE**, transversal a todas las marcas. Entregado por Sam como archivo
>    consolidado y **registrado literal** —md5 idéntico contra el origen—. **Es cero estado a
>    propósito**: su Parte D lleva las consultas, no las cifras.
> 2. **`skills/INDEX.md`** — v1.12 → **v1.13**, con la fila nueva en la tabla de decisión, la regla
>    de carga nueva, y las notas de versión anteriores **intactas**.
>
> **Nodos del JSON que cambiaron**, y que este mapa refleja sin reescribir su cuerpo: `_meta`
> (versión y `last_session`, con el anterior **archivado íntegro**), el nodo nuevo de primer nivel
> **`carril_regulator`**, y `publish_pipeline._update_2026-09-12`.
>
> **Cuerpo íntegro, cero regeneración:** este archivo lleva notas fechadas y estructura acumulada que
> **no es derivable del JSON**, y **no existe generador en el repo** — «regenerar» a mano sería
> reescribir con interpretación y borrar historia (`CC_PROTOCOL.md` §0). Regla escrita en
