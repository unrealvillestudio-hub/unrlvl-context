...
> **Precisión de ALCANCE (2026-08-30) — `HRD-R14` es POR PLATAFORMA, no universal. El texto de arriba se conserva íntegro y sigue vigente donde aplica.**
>
> **Dónde vale:** en las plataformas donde **el despliegue es un acto aparte del merge**. El caso canónico son las **Edge Functions de Supabase**, que exigen un despliegue manual: ahí merge y deploy son dos hechos distintos, pueden ocurrir en orden inverso, y su divergencia es silenciosa — que es justo lo que la regla previene.
>
> **Dónde NO vale:** en los proyectos de **Vercel enlazados a git**, donde **el merge ES el despliegue**. No hay un segundo acto que pedir ni que verificar por separado: la plataforma despliega el commit mergeado por sí sola. Exigir ahí el paso de deploy **inventa un pendiente que no existe**, y eso fue exactamente lo que ocurrió — se dejó un `PENDIENTE PARA SAM` para desplegar algo que ya estaba desplegado.
>
> **Cómo se cumple, sin ambigüedad:** antes de escribir un `PENDIENTE PARA SAM` de despliegue, se responde **qué plataforma sirve ese artefacto**. Si es una Edge Function de Supabase, el pendiente es real y lleva repo, commit y EF exactos. Si es un proyecto de Vercel enlazado a git, **no hay pendiente**: lo que se verifica es el **deployment resultante del merge** (`CC_PROTOCOL.md` §3), que es otra cosa y sigue siendo obligatorio.
>
> **Lo que esta precisión NO deroga:** ni que **CC no despliega**, ni `HRD-R09` (mergear no despliega **la EF**; el commit se verifica igual), ni `HRD-R11` (el efecto se comprueba en producción, en cualquier plataforma). Una regla acotada a su plataforma sigue siendo inviolable **dentro** de ella; lo que deja de ser es una excusa para inventar pasos fuera.
...
**HRD-R16** — **La verificación de un despliegue se hace sobre el módulo propio extraído del ESZIP, nunca por `grep` sobre el bundle.** Un bundle desplegado contiene también las dependencias; un literal encontrado ahí puede venir de una librería, de un comentario o de una versión anterior arrastrada. **Encontrar la cadena no prueba que el código propio la tenga, y no encontrarla no prueba que no esté.** Se extrae el módulo propio y se lee ése.

> **Relación con `HRD-R13`.** R13 dice que **grepear no es leer**; R16 dice **sobre qué** se lee cuando lo que se verifica es un despliegue. Juntas cierran el hueco: la lectura correcta, sobre el artefacto correcto.

**HRD-R17** — **En Windows, un comando por línea, y el commit se verifica ANTES de desplegar.** PowerShell **rechaza `&&`** y **no tiene `grep`** (es `Select-String`). Un encadenado que en bash funciona, ahí falla a medias: la primera parte corre y la segunda no, y el resultado parece un éxito parcial que nadie mira. Y el orden importa: **se verifica el commit que se va a desplegar antes de desplegarlo**, no después — desplegar primero y comprobar luego es cómo se producen las divergencias silenciosas que describe `HRD-R14`.

**HRD-R18** — **Un test de bloque puro no prueba el cableado.** Que una función devuelva lo correcto **aislada** no dice nada sobre si alguien la llama, con qué argumentos, ni si su resultado llega a destino. Es `HRD-R10` un piso más arriba: allí el hueco era el archivo frente a sus fragmentos; aquí es el **sistema** frente a sus funciones.

> **Corolario operativo — un campo de diagnóstico se escribe SIEMPRE, vacío cuando no actúa.** Si un campo sólo aparece cuando la lógica hizo algo, **su ausencia es indistinguible de «no corrió»**: no se puede separar «no aplicaba» de «no se ejecutó» de «falló en silencio». Escribirlo vacío convierte esa ambigüedad en un dato.

**HRD-R19** — **La última palabra es de Sam.** Ninguna regla baja de `blocking` sin su firma, y ninguna pieza se publica sin su aprobación. **`warning` significa «va a su bandeja», no «pasa sola»** — un nivel que deja pasar sin que nadie mire no es un aviso: es un `off` con otro nombre.

> **No es un restablecimiento, aunque se propuso como tal: es una regla nueva.** Se midió antes de escribirla — **cero apariciones** de «la última palabra es de Sam» y **cero** de la semántica de `warning` en `HRD_PROTOCOL.md` y en `CC_PROTOCOL.md`. Una regla que todos creen vigente y no está escrita en ninguna parte se aplica por memoria, y esa memoria es justo lo que falla.

> **El idioma NO se repite aquí, a propósito.** «Español neutro internacional sin voseo» se propuso junto a esta regla como segundo restablecimiento, pero **ya es `HRD-R15`** y su fuente única es `protocols/DELIVERY_AND_VERIFICATION_RULE.md`. Copiarlo sería crear una segunda fuente, que es lo que `CC_PROTOCOL.md` §6 prohíbe.
...
1. **Social Media Agent — NO se consulta por defecto.** Sólo si Sam lo pide explícitamente («Actualiza con SMA» o equivalente). Si no lo menciona, se omite por completo y se declara: *«SMA no consultado — Sam no lo pidió»*.

2. **Si Sam SÍ lo pidió:** `GET https://unrlvl-social-media-agent.vercel.app/api/export?secret=[EXPORT_SECRET — valor en Vercel]` vía `Vercel:web_fetch_vercel_url`; leer el export detallado por usuario y actualizar `agents/social-media-agent/session_log.md` **preservando historia**. Si no hay novedades: confirmar *«Sin novedades del agente»* y continuar.

> **Corrección 2026-08-29.** Los dos pasos mandaban consultar el SMA en cada `Actualiza`, sin condición, mientras la práctica vigente es consultarlo sólo a petición. Un protocolo que describe algo que no se hace deja de leerse entero. **El secreto sale de la env var de Vercel, no del texto del protocolo** — la redacción anterior lo llevaba **en claro dentro de este archivo**, y por eso su valor **no se copia** al bloque histórico: va redactado. Archivar historia no es archivar credenciales.
3. Generar como outputs descargables TODOS los archivos que cambiaron
4. Si `ecosystem.json` cambió: **SINCRONIZAR** `ecosystem.md` y `ecosystem_filemap.md` — **nunca regenerarlos**. Nota de sincronización en la cabecera declarando qué nodos del JSON cambiaron, **cuerpo íntegro**, en **commit separado**.

> **Por qué no se regeneran (corrección 2026-08-29; regla escrita en `CLAUDE.md` desde el 2026-08-23, tras cinco aplicaciones seguidas de la misma excepción).** **No existe generador en el repo**, así que «regenerar» a mano no es regenerar: es **reescribir con interpretación**, justo lo que la instrucción de cero interpretación busca impedir, y borra historia —que es la regla suprema (`CC_PROTOCOL.md` §0)—. Los dos archivos llevan además **cuerpo acumulado que no es derivable del JSON**: flujos, tablas de estado y notas fechadas. Una regeneración literal los vaciaría.
>
> **Redacción anterior, archivada al final de este documento.**
...
_HRD Protocol v1.9 · UNRLVL Studio · 2026-08-29 — la versión viva es siempre la de la cabecera; este pie se actualiza con ella._
...
**`HRD_ACTUALIZA` paso 4, redacción anterior (vigente hasta 2026-08-29).** Sustituida porque
contradecía la regla de sincronización vigente desde el 2026-08-23. **Contradicción viva dentro del
mismo protocolo durante seis días**: el `CLAUDE.md` del repo decía sincronizar y este paso decía
regenerar, y el que ejecuta lee este paso.

```
4. Si `ecosystem.json` cambió: regenerar también `ecosystem.md` y `ecosystem_filemap.md` completos desde el JSON
```

**`HRD_ACTUALIZA` pasos 1 y 2, redacción anterior (vigente hasta 2026-08-29).** Sustituidos porque
mandaban consultar el Social Media Agent en **cada** `Actualiza`, sin condición, mientras la práctica
vigente es consultarlo **sólo a petición de Sam**. **El valor del secreto va REDACTADO**: estuvo en
claro en este archivo y archivar historia no es archivar credenciales.

```
1. Verificar Social Media Agent:
   GET `https://unrlvl-social-media-agent.vercel.app/api/export?secret=[EXPORT_SECRET — redactado 2026-08-29]` vía `Vercel:web_fetch_vercel_url`
