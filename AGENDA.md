...
_Actualizada: 2026-07-01 · v2026-07-01 (#47 Expert/Boids: E3 CAPTURA COMPLETO end-to-end — E3b-2 front signed upload + E3b-3 EF genérica de barrido de huérfanos + E3b-4 prueba real de Marisol desde su dispositivo, todas verdes. Marisol captura en producción. Próximo: E5 front Expert completo + decisión E4. INCIDENTE detectado: content-dispatcher-poll 592 fallos desde 17-jun por ambigüedad de trigger_iid_agent — dejado a chat R4B. · base previa v2026-06-28-v3)_
...
## 🔵🔵 SPRINT #47 — Modo Expert/Boids — DISEÑADO Y CERRADO (27-jun) · E1-E3b construido · E3 captura COMPLETO
...
- **A — Vía D → server-side (post-HEVC): frames con ffmpeg en el servidor + Cloud Vision OCR.** El informe E3-exploratorio mató la EF self-contained (runtime sin subprocess + cap 2s CPU + bundle 20MB → ffmpeg/tesseract in-EF imposible). El canvas en el navegador falló con HEVC → se movió a `/api/extract-frames` (ffmpeg server-side, decodifica cualquier códec). El video sube al bucket vía signed upload URL, transita segundos, se borra. OCR vía **Google Cloud Vision** (DOCUMENT_TEXT_DETECTION) reusando la **credencial Vertex existente** (cero proveedor nuevo).
...
| E2 | Storage | bucket `iid-expert-uploads` privado (PROTAGONISTA del flujo server-side) | ✅ **LIVE (27-jun)** |
| E3-EF | EF OCR | `iid-expert-ocr` v1 en `unrlvl-iid-functions` (Vía D: frames + Cloud Vision). Smoke verde | ✅ **v1 LIVE + smoke (27-jun)** |
...
| **E3b** | **Server-side (rediseño post-HEVC)** | extracción con ffmpeg server-side. Diseño cerrado (ver DISENO_E3_server_side + session_log 28-jun b) | ✅ **COMPLETO (1-jul)** |
...
| E3b-2 | front | `ExpertCapture.tsx` reescrito + `/api/sign-upload` (signed upload URL server-side, REST sin SDK). PR #4 mergeado. | ✅ **CERRADO — E2E verde con HEVC de Marisol (1-jul)** |
| E3b-3 | cron huérfanos | EF genérica `storage-orphan-sweep` (parametrizable, REST-only por trigger protect_delete, recorrido recursivo). PR #8 mergeado+deployado. Crons jobid 35 (iid-expert 1h) + 36 (arreglo job 32 roto). | ✅ **CERRADO — barrió 10/11 huérfanos reales (1-jul)** |
| E3b-4 | prueba Marisol | Marisol capturó desde SU dispositivo con su HEVC, desde cero (fila 3c40f492, 15 frames, 601 OCR, bucket en 0). Gate de cierre E3. | ✅ **CERRADO — VERDE (1-jul)** |
...
| E5 | Front Expert | sub-pestaña Basic/Expert completa (envuelve E3b-2, reemplaza mount temporal "Expert (prueba)"). Rama+PR+Preview. **PRÓXIMO FOCO de #47.** | 🔵 PRÓXIMO |
...
`content-dispatcher-poll` (cron jobid 29) lleva **592 fallos consecutivos desde 17-jun 10:00**, cero éxitos en 14 días. `iid-brief-biweekly` (jobid 2) falló hoy 1-jul 7AM. **Causa raíz:** `intel.trigger_iid_agent` tiene dos overloads (text) y (text,jsonb); los crons la llaman con literal sin cast → `function is not unique` → el job no dispara. El carril automático de contenido está PARADO hace 2 semanas (la EF content-dispatcher v27 está sana, pero nadie la invoca). **Fix:** castear literal a `::text`; recrear crons con cron.schedule. Los ~24 crons iid-*-research/process (active:false) tienen el mismo bug latente. Bajo checkpoint HRD. **Dejado al chat de R4B/dispatcher (brief entregado 1-jul).** LECCIÓN: "active:true" ≠ "funcionando".
...
| ✅ MERGEAR PR #4 (E3b-2) + PR #8 (E3b-3) | ambos mergeados 1-jul; storage-orphan-sweep deployada + secret cargado | E3b-2/3 |
| ✅ PRUEBA REAL de Marisol (E3b-4) | VERDE 1-jul — Marisol capturó desde su dispositivo con su HEVC (fila 3c40f492). E3 CERRADO. El bucket E2 queda como protagonista del flujo server-side | cierre E3 ✅ |
| 🟡 Rotar STORAGE_SWEEP_SECRET | se pegó en chat durante la prueba (blast radius mínimo); regenerar + actualizar command de crons 35/36 | higiene |
...
| Aprobar learnings Professor | ✅ 28 ventana IID (24+27-jun) + 6 de E3b-1 (+ 3 28-jun b) (28-jun c) + 8 (1-jul) aprobados · 22 pendientes (mayo) | Professor |
...
| 43 | **Versionar EFs del IID en repo (deuda sin-repo §1)** — PARCIALMENTE saldado (iid-inbound versionado en unrlvl-iid-functions, PRs #1-#5; storage-orphan-sweep en el mismo repo, PR #8). Falta el resto de EFs IID (iid-core, fanout.ts, content-*, etc.) → mismo repo. NOTA: la EF de #47 (iid-expert-ocr, E3) nace versionada en este repo. | UNRLVL |
...
| 47 | ✅ **Modo Expert/Boids — E3 CAPTURA COMPLETO end-to-end (1-jul)** — E3b-1/2/3/4 cerradas. Marisol captura en producción desde su dispositivo. Próximo: E5 front Expert completo + decisión E4. Ver session_log §9 (1-jul). | UNRLVL |
...
- ✅ **#47 Expert/Boids — E3 CAPTURA COMPLETO end-to-end (1-jul).** E3b-2 (front signed upload URL server-side, REST sin SDK, PR #4) + E3b-3 (EF genérica storage-orphan-sweep, REST-only por trigger protect_delete, recorrido recursivo por list folder-aware, PR #8 + crons 35/36 arreglando el job 32 roto) + E3b-4 (Marisol capturó desde su dispositivo con su HEVC, fila 3c40f492, bucket en 0). Gotcha corregido: sign-upload rechazaba con 400 por Content-Type:application/json sin body. Hallazgo: trigger protect_delete bloquea DELETE FROM storage.objects (job 32 llevaba semanas fallando en silencio). Incidente detectado (dispatcher, dejado a R4B). Professor: 8 learnings. Próximo: E5 + decisión E4. — 2026-07-01
...
**#47 Expert/Boids — estado 2026-07-01 (E3 CAPTURA COMPLETO end-to-end):** subsistema PERMANENTE de onboarding. 2 fases: Fase 1 (captura+OCR) E1-E8; Fase 2 (calibración) = skill `genome-calibration`. **Arquitectura server-side (post-HEVC):** navegador pide signed upload URL a `/api/sign-upload` (service_role, REST) → sube video directo al bucket `iid-expert-uploads` (E2) → `/api/extract-frames` (ffmpeg-static en Orchestrator/Vercel, Flujo A) decodifica CUALQUIER códec → frames → EF `iid-expert-ocr` v1 INTACTA → OCR Cloud Vision → persiste captured_techniques. ffmpeg borra el video tras extraer + EF genérica `storage-orphan-sweep` (cron cada hora) como backup. Anti-IP: video TRANSITA (segundos), no persiste. **E3 CERRADO end-to-end** — E3b-4: Marisol capturó desde SU dispositivo con su HEVC (fila 3c40f492, 15 frames, 601 OCR chars, bucket en 0). Estados: E1 tabla LIVE, E2 bucket LIVE (protagonista), E3-EF v1 LIVE (INTACTA), E3b-1/2/3/4 cerradas. **PRÓXIMO: E5 (front Expert completo que reemplaza el mount temporal "Expert (prueba)") + decisión E4 (absorber o mantener).** Fase 2 (E6/E7/E8) no bloquea captura. Marisol captura Y calibra en su scope, NUNCA Lucien/UNRLVL; Sam firma el INSERT.

**Storage barrido (infra primaria, 1-jul):** EF `storage-orphan-sweep` (unrlvl-iid-functions, genérica, parametrizable bucket+older_than_minutes+prefix) barre huérfanos de cualquier bucket temporal. REST-only obligatorio (trigger `protect_objects_delete` bloquea DELETE FROM storage.objects directo con 42501). List folder-aware / no recursivo → recorrido recursivo. Auth por header `x-sweep-secret` (verify-JWT OFF en la EF). Crons: jobid 35 iid-expert-orphan-sweep (0 * * * *, iid-expert-uploads >60min) + jobid 36 unrlvl-media-temp-cleanup (0 3 * * *, >12 días, reemplaza el job 32 roto que fallaba en silencio).
...
**IID — estado 2026-06-25 a:** content-dispatcher v27 (.limit(1) intacto, transporta domain) → content-run-stage v41 → content-watcher v5 (6 gates; Gate 7/8 eje B NO implementados) → approve-piece v14. Modelo claude-sonnet-4-6. **NOTA 1-jul: el cron que invoca content-dispatcher (jobid 29) está ROTO desde 17-jun (ver INCIDENTE en R4B) — la EF está sana pero no se dispara.**
...
**Secrets:** cada marca su key Resend. Vertex SA en Vercel image-lab + Supabase EF. Auth Sembrador: ORCHESTRATOR_NSCF_IID_INTEL_USERS + _JWT_SECRET en Supabase. Barrido Storage: STORAGE_SWEEP_SECRET (rotar — se expuso en chat 1-jul).
