...
_Actualizada: 2026-06-28 · v2026-06-28-v1 (#47 Expert/Boids: E3-FRONT construido [PR #2] + prueba E2E exitosa desde Preview por Sam · PENDIENTE para cerrar E3: prueba real desde el dispositivo de Marisol · base previa v2026-06-27-v3)_
...
| E3-FRONT | Front extracción | módulo canvas en `Orchestrator` (extrae frames, ~720px, envía a la EF) | ✅ **construido + E2E verde desde Preview (Sam) · PR #2 · PENDIENTE: prueba dispositivo Marisol** |
| E4 | iid-inbound | acciones `expert_*` (auth dos ejes reusada) — REVISAR: E3-EF ya hace la captura, evaluar si E4 sigue siendo necesaria o se absorbe | 🔵 a revisar |
| E5 | Front Expert | sub-pestaña Basic/Expert completa (envuelve E3-FRONT). Rama+PR+Preview | 🔵 mapeado |
...
| 🔴 ROTAR contraseñas temporales Sembrador (SUBE PRIORIDAD) | la registrada en contexto NO coincide con la real (hallazgo smoke E3); credenciales expuestas en sesiones. Script local, recargar secret usuarios | producción real Marisol |
| 🔵 PRUEBA REAL desde el dispositivo de Marisol (cierra E3) | Mergear PR #2 → Marisol prueba desde SU dispositivo, desde cero (prog. mañana 28-jun). Si iPhone: validar canvas en Safari iOS (riesgo conocido). Verde → limpiar bucket E2 | cierre E3 + destino E2 |
...
| Aprobar learnings Professor | ✅ 28 ventana IID (24+27-jun) aprobados · 22 de mayo pendientes (listados, a tu criterio) | Professor |
...
| 47 | 🔵 **Modo Expert/Boids — E1+E2+E3-EF+E3-FRONT construidos (28-jun) · E2E verde desde Preview · pendiente prueba Marisol** — subsistema de onboarding, 2 fases. Vía D (frames navegador canvas + Cloud Vision OCR). PR #2 (Orchestrator) con mount temporal "Expert (prueba)" en SeederShell (reemplazar en E5). Próximo: mergear #2 → prueba dispositivo Marisol (cierra E3, decide E2) → E4 (revisar) → E5-E8. Ver bloque SPRINT #47 + session_log §9 (28-jun). | UNRLVL |
...
- ✅ **#47 Expert/Boids — E3-FRONT construido + E2E verde desde Preview (28-jun).** Componente de extracción de frames con canvas nativo en Orchestrator (`ExpertCapture.tsx` + `iidExpert.ts`), mount temporal "Expert (prueba)" en SeederShell (reemplazable por E5). Caption persistido en source_refs (opción a: elemento del array, pasa el guard Array.isArray de la EF). PR #2 (rama claude/optimistic-kapitsa-090073). **Prueba E2E exitosa desde el Preview por Sam:** video real (720×1280, ~30s) → 15 frames extraídos → Cloud Vision leyó 1115 chars de calidad (estructura de divulgación visible) → fila en captured_techniques (captured_by=marisol, scope NeuroneSCF validado). Fila de prueba borrada. **PENDIENTE para cerrar E3:** prueba real desde el dispositivo de Marisol (prog. 28-jun, desde cero). Sin learnings nuevos (la prueba confirma learnings ya aprobados). — 2026-06-28
...
**#47 Expert/Boids — estado 2026-06-28 (E3-FRONT construido + E2E verde):** subsistema PERMANENTE de onboarding de marcas. Dos fases: Fase 1 (captura+OCR) se construye E1-E8; Fase 2 (calibración por convergencia = método 10 textos de Lucien) = skill conversacional `genome-calibration`. **Arquitectura Vía D:** navegador extrae frames con canvas nativo → ~720px JPEG en el body → EF `iid-expert-ocr` v1 → Cloud Vision OCR (credencial Vertex) → persiste solo texto en `intel.captured_techniques`. El video ajeno nunca toca la infra. E1 tabla LIVE. E2 bucket `iid-expert-uploads` CONDICIONAL (limpiar tras prueba Marisol). E3-EF v1 LIVE + smoke verde. E3-FRONT construido (PR #2, `ExpertCapture.tsx`), **probado E2E con éxito desde el Preview por Sam** (15 frames, 1115 chars OCR, fila persistida). Caption en source_refs (elemento del array). **PENDIENTE para cerrar E3: prueba real desde el dispositivo de Marisol (28-jun, desde cero); si iPhone, validar canvas en Safari iOS).** Verde → limpiar E2 → E4 (revisar si E3-EF la absorbe) → E5-E8. Marisol captura Y calibra en su scope, NUNCA Lucien/UNRLVL; Sam firma el INSERT a brand_voice_genome.
...
**Anti-IP (dos modos de semilla):** Basic = material nunca leído, tema neutro. Expert/Boids = material es insumo de aprendizaje de TÉCNICA, nunca fuente a reescribir (análogo frame Nietzsche de Lucien: motor interno, nunca citado). En Expert (Vía D) el video ajeno NUNCA toca la infra — el navegador lo decodifica y extrae frames localmente; solo suben frames-imagen que la EF OCRea y descarta; persiste solo texto-método. Anti-IP máximo.
