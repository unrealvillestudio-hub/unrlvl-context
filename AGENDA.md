...
### ✅ CUMPLIDO — y el número esperado y el medido coinciden exactamente

**21 latidos de 21 posibles** —3 al día por 7 días, **ni uno perdido**— del **2026-09-19 18:11:01**
al **2026-09-26 10:11:00**, en **8 fechas distintas**, con el proyecto FPHS en **`ACTIVE_HEALTHY`**
[`medido`]. **21 de 21 significa cero corridas fallidas del cron en una semana**, así que no hizo
falta abrir los logs de Vercel.

**«Tres peticiones diarias bastan» deja de ser `deducido` y pasa a `medido`.** Era la última
etiqueta blanda del frente: la documentación de Supabase dice «a few … each day» **sin dar número**,
y el tres se eligió con margen, **declarado `deducido` desde el primer día** para no darlo por
cierto. Ahora hay una ventana completa que lo respalda.

🟡 **La mitad que CC NO puede medir, y se escribe en vez de callarse.** El criterio tenía **dos**
partes —proyecto activo **y** sin correo de aviso—. **CC sólo midió la primera**: no lee el buzón de
Sam y `MAIL_PRIVACY_RULE` lo impide. Esa mitad queda **`reportado` por Sam o sin verificar**.
Declarar cumplido el conjunto sin la nota habría sido **afirmar sin medir justo la mitad**. Lo que
sí sostiene el resultado: **un proyecto pausado no aparece `ACTIVE_HEALTHY`**.
...
## 🗓️ CIERRE 2026-09-21-v1 — El ciclo comercial se vuelve procedimiento, y una credencial encontrada no se usa

_(Bloque al tope. **No reescribe ninguna versión anterior** — el `v2026-09-20-v2` y toda su cadena
quedan íntegros inmediatamente debajo. Lo etiquetado `reportado` lo afirma el **brief de Claude.ai
del 2026-09-21**; lo etiquetado `medido` lo consultó CC el mismo día. **Sin datos de ningún
prospecto**, por `protocols/MAIL_PRIVACY_RULE.md`. Detalle en `brands/ForumPHs/session_log.md`,
entrada del **2026-09-21**.)_

---

### ✅ CERRADO HOY

**El procedimiento comercial completo queda formalizado** en
`brands/ForumPHs/sales-kit/PLAYBOOK_propuesta_prospecto_PH.md`, copiado **literal** y **verificado
por md5** [`medido`]. Es **la pieza principal del kit** y manda sobre las demás.

**Modelo de cartera definitivo** — recuperación incluida sin costo hasta 90 días; desde 90 días,
recomendación de vía legal y **expediente sin costo** con la gestión a cargo de la Junta; **sin
comisión sobre lo recuperado en ninguna etapa**; sin representación judicial [`reportado`].
🔴 **Sustituye al honorario porcentual del 2026-09-19**, que queda bajo guard `⛔ NO OPERATIVO` **en
dos sitios**: el `INDEX.md` del kit **y dentro de `correo_que_da_precio.md`**, que es donde el
modelo viejo está escrito — marcar sólo el índice habría dejado la pieza ofreciendo un porcentaje
que ya no existe.

**Regla nueva en `CC_PROTOCOL.md` §15 (el protocolo pasa a v13): una credencial encontrada NO se
usa, se reporta** — por su nombre y ubicación, **nunca por su valor**.

---

### 🔴 DEUDAS ABIERTAS

1. 🔐 **SEGURIDAD — `media-store` Y `meta-graph-post` llevan el MISMO secreto cableado** como valor por defecto de
   `Deno.env.get(...)`, con **`verify_jwt: false`** y escritura con **`service_role`** [`medido` por
   CC el 2026-09-21; **el valor no se transcribió a ningún archivo ni se usó**]. Hay que **rotarlo**,
   dejarlo sólo en la variable de entorno y **quitar el valor por defecto**. **Requiere brief propio
   con test de la marca N+1** — es despliegue sobre una EF compartida y no se ejecuta en un Actualiza.
   **Patrón, no caso:** un `?? 'valor'` sobre un secreto convierte un fallo de configuración en un
   agujero silencioso; **un secreto sin variable debe ser fail-loud**.

   **⚠️ CORREGIDO el 2026-09-22:** no es una EF, son **dos**, y **comparten variable y literal**
   —rotar una sola deja la otra rechazando el secreto nuevo—. Y **no se detectó el 21: ya estaba en
   esta AGENDA el 2026-09-09**, punto 6 de aquella sesión. **Lleva 13 días abierto.**

   **Orden de rotación que no rompe nada** [lo medido que lo permite: **ningún cron** las invoca,
   **ni una** invocación en `net._http_response` y **cero** en 24 h de `edge_logs`, así que **no hay
   llamador automático** que cortar]:

   1. **Sam** mira si `MEDIA_STORE_SECRET` existe en los secretos del proyecto. **Sólo él puede** —
      CC no lee variables de entorno (`CAPABILITIES` 1.18) y **no lo averigua usando el literal
      hallado**, por `CC_PROTOCOL` §15. **Si no existe, la ponen primero con el valor actual**, para
      que retirar el literal no cambie nada.
   2. **Sam** genera un valor nuevo y lo pone en `MEDIA_STORE_SECRET`. **Nunca el literal viejo:**
      rotar no es mover el mismo valor de sitio — ese ya está en el código desplegado.
   3. **CC** despliega las **dos** EF sin el `?? literal`, con fail-loud si falta la variable, y
      **pasando `verify_jwt: false` EXPLÍCITO**.
   4. **Verificación por efecto:** una invocación real con el secreto nuevo devuelve `200`; una con
      el viejo devuelve `401`. **Las dos comprobaciones**, o no se sabe si rotó o si sólo dejó de
      funcionar.

   🔴 **LA TRAMPA QUE ROMPERÍA TODO:** la tool `deploy_edge_function` tiene **`verify_jwt` con
   DEFAULT `TRUE`** [`medido` sobre su esquema el 2026-09-22]. Las dos EF corren con `false` y se
   autentican por cabecera: **un despliegue que no pase `verify_jwt: false` explícito las deja
   exigiendo JWT y todo llamador recibe 401**. Es el `--no-verify-jwt` de §10 por la vía de la tool.
2. ⛔ **LOS ÁLAMOS — DESCARTADO el 2026-09-22, y no por haberse resuelto.** Sam corrigió la base
   [`reportado`]: **el gasto operativo de los últimos siete meses está inflado por prestaciones no
   provisionadas por el socio anterior**, que ahora se sanean. La cifra de pérdida **mide ese
   saneamiento, no el contrato**, así que no sirve como diagnóstico operativo. **Y la decisión es de
   Ivette, no de UNRLVL.** Se hará un análisis más preciso **más adelante, no ahora**. La conclusión
   del 2026-09-19 queda bajo guard en `session_log.md`, **archivada y no borrada**.
3. **Documento de decisión interno** — su sección de cartera describe el honorario ya sustituido.
4. **La Suite deja deducir 143 unidades** dividiendo $4,862 entre $34. Quitarlo **exige rehacer el
   cálculo**, no basta con borrar la cifra.
5. **Frecuencia de visitas** comprometida en el modelo de servicio: **sigue sin definirse** desde el
   2026-09-19. Es compromiso contractual **con costo**.
6. 🔍 **PRIVACIDAD — datos de prospecto escritos en context files.** Detalle con archivo y línea en
   el cuerpo del PR de este Actualiza. **CC no borra nada: la decisión es de Sam**, porque choca de
   frente con la regla de no borrar historia.

---

### 🔍 LO QUE CC CORRIGE POR MEDICIÓN — dos cosas del brief

1. **El Professor trae 16 learnings, no 14.** Medido sobre `professor_learnings` con
   `session_date = '2026-09-21'`: **16 filas, las 16 aprobadas**, **3** con prefijo `SALES-KIT` y
   **2** con `SUPERSEDES`, frente a los «14, dos y uno» del brief. No cambia nada de lo escrito,
   pero **un recuento afirmado y no medido es el defecto que ya costó una corrección el 2026-09-19**.
2. **El pendiente del keepalive YA ESTÁ CERRADO y no se copia como deuda viva.** El brief lo lista
   abierto —«el de `pg_cron` no evita la pausa»—, y eso **dejó de ser cierto el 2026-09-20**: el
   cron externo está en producción con **tres latidos verificados** y el `pg_cron` interno **fue
   retirado**. Lo único vivo de ese frente es **el criterio de 7 días, que va por el día 2**.
