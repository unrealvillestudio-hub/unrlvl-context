...
_Actualizada: 2026-06-27 · v2026-06-27-v1 (#48 Approval por email COMPLETO y verificado en vivo, iid-inbound v9 · próximo: #45 brand_topics 6 marcas Marisol [BLOQUEANTE] + sprint #47 Expert/Boids · base previa v2026-06-26-v2)_

---

## 🔴🔴 FOCO INMEDIATO — #45 brand_topics Marisol (BLOQUEANTE) + sprint #47 Expert/Boids + R4B
...
## 🔵🔵 PRÓXIMO SPRINT — #47 Modo Expert/Boids (construcción de marcas y genomas)

**Qué es:** segundo modo de captura del Sembrador, para construir marcas/genomas con análisis profundo de técnica de creadores (el método del caso boids original que produjo la voz educativa de Lucien). Uso INTENSIVO ahora (mientras se construyen las marcas de Patricia), casi nulo después.
...
**Decisiones de diseño a tomar con Sam ANTES de construir (no asumir):**
- Dónde corre el análisis (framing/OCR/Whisper): ¿EF, lab, o procesamiento asistido por Claude como en el caso boids original? (En boids lo hizo Claude: framing+OCR de frames; Whisper estuvo bloqueado por allowlist de red del sandbox.)
- Output del modo Expert: ¿se queda como insumo que Sam lee y usa para diseñar el genoma (asistido, recomendación probable), o se intenta automatizar la creación de genoma? El output NO es un tema neutro como Basic — es un análisis de MÉTODO/TÉCNICA. Definir cómo se persiste (¿tabla de "técnicas capturadas"? ¿alimenta un genoma en construcción?).
- Upload: dónde se almacena el archivo (Storage de Supabase, temporal, etc.).
...
| Aprobar learnings Professor | (5 del 25-jun b pendientes + 7 del 26-jun ✅ + 5 del 27-jun pendientes) | Professor |
...
| 43 | **Versionar EFs del IID en repo (deuda sin-repo §1)** — PARCIALMENTE saldado (iid-inbound versionado en unrlvl-iid-functions, PRs #1-#5). Falta el resto de EFs IID (iid-core, fanout.ts, content-*, etc.) → mismo repo. | UNRLVL |
...
| 47 | **Sprint modo Expert/Boids** — segundo modo de captura del Sembrador (sub-pestaña Basic/Expert + upload video/imagen + framing/OCR/análisis de técnica + seed pedagógico lane). Construcción de marcas/genomas (el método del caso boids). Uso intensivo ahora, nulo después. Requiere decisiones de diseño con Sam antes de construir. Detalle en bloque "PRÓXIMO SPRINT". | UNRLVL |
