...
_Actualizada: 2026-06-27 · v2026-06-27-v3 (#47 Expert/Boids: E1+E2+E3-EF construidos y verificados [tabla captured_techniques + bucket + EF iid-expert-ocr v1 LIVE + smoke verde], Vision API habilitada · próximo: E3-FRONT + E4-E8 · base previa v2026-06-27-v2)_
...
| E3-FRONT | Front extracción | módulo canvas en `Orchestrator` (extrae frames, ~720px, envía a la EF) | 🔵 PRÓXIMO |
...
| 47 | 🔵 **Modo Expert/Boids — E1+E2+E3-EF construidos (27-jun) · construyendo E3-FRONT+E4-E8** — subsistema de onboarding, 2 fases. Vía D (frames navegador canvas + Cloud Vision OCR). E1 tabla LIVE, E2 bucket condicional, E3-EF `iid-expert-ocr` v1 LIVE+smoke verde, Vision habilitada. Próximo: E3-FRONT (extracción canvas en Orchestrator) → prueba Marisol. Ver bloque SPRINT #47 + session_log §9 (27-jun c). | UNRLVL |
...
- ✅ **#47 Expert/Boids — E1+E2+E3-EF construidos y verificados (27-jun c).** Informe E3-exploratorio mató la EF self-contained (runtime sin subprocess + 2s CPU + 20MB bundle) → arquitectura **Vía D** (frames del navegador con canvas nativo + Cloud Vision OCR reusando credencial Vertex). E1 tabla captured_techniques LIVE. E2 bucket iid-expert-uploads privado (CONDICIONAL, candidato a limpieza tras prueba Marisol). E3-EF `iid-expert-ocr` v1 LIVE (PR #6) + fix des-escapado PEM (PR #7) + deploy con paridad git↔deploy + **smoke verde** (PEM→Vision→insert sano, scope 403, body 400, texto OCR coincide). Vision API habilitada por Sam. Hallazgo seguridad: contraseña temporal Marisol del contexto NO coincide con la real → rotar (sube prioridad). Professor: 6 learnings (+28 ventana IID aprobados). — 2026-06-27
...
**#47 Expert/Boids — estado 2026-06-27 (E1+E2+E3-EF construidos):** subsistema PERMANENTE de onboarding de marcas. Dos fases: Fase 1 (captura+OCR) se construye E1-E8; Fase 2 (calibración por convergencia = método 10 textos de Lucien) = skill conversacional `genome-calibration`. **Arquitectura Vía D** (informe E3-exploratorio mató EF self-contained): el navegador extrae frames con canvas nativo (sin instalar nada) → ~720px JPEG en el body → EF `iid-expert-ocr` v1 → Cloud Vision OCR (credencial Vertex reusada) → persiste solo texto en `intel.captured_techniques`. El video ajeno nunca toca la infra. E1 tabla LIVE. E2 bucket `iid-expert-uploads` privado CONDICIONAL (candidato a limpieza tras prueba Marisol). E3-EF v1 LIVE + smoke verde. Vision habilitada. PRÓXIMO: E3-FRONT (extracción canvas en Orchestrator, sesión CC apuntada a ese repo) → prueba Marisol (cierra E3, decide E2) → E4-E8. Marisol captura Y calibra en su scope (6 marcas), NUNCA Lucien/UNRLVL; Sam firma el INSERT a brand_voice_genome.
