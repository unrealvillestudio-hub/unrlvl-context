...
| 69 | **Operación B — consolidación de IDs de marca PO (NUEVO 6-jul, mapeada por CC).** Tres consolidaciones: (a) fundir PatriciaOsorioComunidad→PatriciaOsorioConectando (repunte fila-por-fila; OJO colisión UNIQUE en brand_context_cache → borrar cache de Comunidad primero); (b) renombrar PatriciaOsorioVizosSalon→VizosSalon (crear-nuevo-y-mover en transacción: 28 FKs en public.brands + 7 FKs en crm.orgs, TODAS ON UPDATE NO ACTION; incluye repuntar la sesión de calibración sembrada + refs jsonb embebidas); (c) retirar PatriciaOsorioPersonal (status=retired, NO borrar). SUPERFICIE DOBLE: además de DB, las 3 IDs están hardcodeadas en 8 archivos across 7 repos (Orchestrator, AgentLab, WebLab, VoiceLab, ImageLab×2 copias, VideoLab, unrlvl-ops, SocialLab) en 2 casings (Pascal + camel). PatriciaOsorioConectando NO existe en código → el merge debe DARLA DE ALTA. Cero riesgo Meta/Shopify (no tienen estas marcas). Riesgo R4 (código desincronizado) = certeza si el deploy no es coordinado. PENDIENTE: runbook SQL transaccional + checklist de código (CC lo prepara sin ejecutar). | UNRLVL/PatriciaOsorio |
