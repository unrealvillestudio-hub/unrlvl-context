...
_Actualizada: 2026-06-28 · v2026-06-28-v2 (#47 Expert/Boids: E3-FRONT-canvas FALLÓ con HEVC en equipo de Marisol → REDISEÑO a server-side total [ffmpeg en /api del Orchestrator]; diseño cerrado, pendiente construcción E3b-1..4 · base previa v2026-06-28-v1)_
...
| E3b-1 | /api ffmpeg | `Orchestrator/api/extract-frames` (ffmpeg-static, handler Node nativo, borra video tras extraer) | 🔵 PRÓXIMO |
| E3b-2 | front | `ExpertCapture.tsx` reescrito: sube video al bucket → llama extract-frames → pasa frames a la EF (intacta) | 🔵 mapeado |
| E3b-3 | cron huérfanos | barre videos del bucket > 1h (backup del borrado) | 🔵 mapeado |
| E3b-4 | prueba Marisol | con el MISMO video HEVC que falló hoy → debe pasar E2E. Gate de cierre E3 | 🔵 mapeado |
...
| 47 | 🔵 **Modo Expert/Boids — E3-FRONT-canvas FALLÓ con HEVC (28-jun) → REDISEÑO server-side** — la extracción en navegador no decodifica H.265 (video de Marisol). Server-side total: ffmpeg en `/api/extract-frames` dentro del Orchestrator → frames → EF iid-expert-ocr INTACTA. Plan E3b-1..4 (prueba final = video HEVC de Marisol). E1+E2+E3-EF siguen LIVE. Ver DISENO_E3_server_side.md + session_log §9 (28-jun b). | UNRLVL |
...
**#47 Expert/Boids — estado 2026-06-28 b (REDISEÑO server-side post-HEVC):** subsistema PERMANENTE de onboarding de marcas. Dos fases: Fase 1 (captura+OCR) E1-E8; Fase 2 (calibración) = skill `genome-calibration`. **La Vía D (extracción canvas en navegador) FALLÓ:** el video de Marisol era HEVC/H.265 y Chrome no lo decodifica. **Rediseño a SERVER-SIDE TOTAL:** navegador sube video al bucket `iid-expert-uploads` (E2) → llama a `/api/extract-frames` (ffmpeg-static, DENTRO del Orchestrator como serverless, NO proyecto nuevo ni ImageLab) → recibe frames → los pasa a la EF `iid-expert-ocr` v1 INTACTA (Flujo A: navegador orquesta, EF probada no se toca) → OCR Cloud Vision → persiste en captured_techniques. ffmpeg borra el video tras extraer + cron de huérfanos. Anti-IP ajustado: el video TRANSITA el bucket (segundos), no persiste; solo persiste texto-método. Patrones cicatriz: handler Node nativo (504), VITE_* build-time, video sube directo por signed URL. E1 tabla LIVE, E2 bucket LIVE (ahora protagonista), E3-EF v1 LIVE+smoke verde (INTACTA), E3-FRONT-canvas obsoleto. Plan: E3b-1 (/api extract-frames) → E3b-2 (front reescrito) → E3b-3 (cron) → E3b-4 (prueba Marisol con el MISMO video HEVC). Todo en repo Orchestrator (1 sesión CC). Marisol captura Y calibra en su scope, NUNCA Lucien/UNRLVL; Sam firma el INSERT.
